...
export default function ApplyEventPage() {
  ...
  const fetchEvents = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/apply-events`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingEventId 
        ? `${process.env.NEXT_PUBLIC_API_URL}/apply-events/${editingEventId}`
        : `${process.env.NEXT_PUBLIC_API_URL}/apply-events`;
  ...
  const handleDelete = async (eventId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/apply-events/${eventId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
